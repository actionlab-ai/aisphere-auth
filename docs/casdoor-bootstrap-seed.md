...
不要把带环境信息的 SQL 提交到仓库。启动时通过参数或 Secret 注入：组织名、应用名、client_id、client_secret、redirect_uri 等。
...
powershell -ExecutionPolicy Bypass -File .\scripts\casdoor\import-casdoor-sql.ps1 `
  ...
  -SeedClientSecret '<替换为真实 OAuth Client Secret>' `
...
bash scripts/casdoor/import-casdoor-sql.sh \
  --seed \
  --seed-output ./deployments/casdoor/sql/aisphere-auth-casdoor.sql \
  --seed-env-output ./.env.casdoor.generated \
  --seed-client-secret '<替换为真实 OAuth Client Secret>' \
  --seed-only
```
...
导入后，`aisphere-auth` 配置应与 seed 参数保持一致。建议直接读取 `--seed-env-output` 生成的 `.env.casdoor.generated`，或者写入配置文件：
...
```bash
bash scripts/casdoor/import-casdoor-sql.sh --seed ...
```
