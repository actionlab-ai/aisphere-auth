...
func (c *HTTPClient) ExchangeCode(ctx context.Context, code string) (*TokenSet, error) {
	...
	return &TokenSet{AccessToken: out.AccessToken, IDToken: out.IDToken, RefreshToken: out.RefreshToken, TokenType: out.TokenType, ExpiresAt: time.Now().Add(time.Duration(out.ExpiresIn) * time.Second)}, nil
}
...
func (c *HTTPClient) GetUserInfo(ctx context.Context, bearer string) (*UserInfo, error) {
	...
	return &UserInfo{ID: firstNonEmpty(out.ID, out.Sub), Owner: owner, Name: name, DisplayName: out.DisplayName, Email: out.Email, Roles: out.Roles, Groups: out.Groups}, nil
}
...
func (c *HTTPClient) Enforce(ctx context.Context, req EnforceRequest) (*EnforceResponse, error) {
	...
	if permissionID == "" { permissionID = c.cfg.PermissionID }
	u, err := url.Parse(strings.TrimRight(c.cfg.Endpoint, "/") + "/api/enforce")
	if err != nil { return nil, err }
	q := u.Query(); q.Set("permissionId", permissionID); u.RawQuery = q.Encode()
	...
	var out struct { Status string `json:"status"`; Msg string `json:"msg"`; Data any `json:"data"` }
	if err := c.doJSON(httpReq, &out); err != nil { return nil, err }
	if out.Status != "ok" { return nil, fmt.Errorf("casdoor enforce status=%s msg=%s", out.Status, out.Msg) }
	allow, ok := out.Data.(bool)
	if !ok { return nil, fmt.Errorf("casdoor enforce returned non-bool data") }
...
func (c *HTTPClient) doJSON(req *http.Request, out any) error {
	...
	if err != nil { return err }
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return fmt.Errorf("casdoor http=%d body=%s", resp.StatusCode, string(body)) }
	if err := json.Unmarshal(body, out); err != nil { return fmt.Errorf("decode casdoor response: %w body=%s", err, string(body)) }
	return nil
}
